        ...
        <div className="relative aspect-[21/9] max-h-[350px] bg-gray-200">
          ...
          <div className="absolute bottom-0 left-0 right-0">
            <div className="max-w-content mx-auto px-4 sm:px-6 pb-8">
              ...
              <p className="text-white/80 text-lg mt-2">{safe?.hero?.subtitle ?? ''}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-content mx-auto px-4 sm:px-6 py-12 space-y-16">
        ...
        >
          <div>
            ...
            <h2 className="font-heading text-3xl font-bold text-text mb-4">{safe?.mission?.title ?? ''}</h2>
            <p className="text-text-light leading-relaxed mb-4">{safe?.mission?.content ?? ''}</p>
            <p className="text-text-light leading-relaxed">{safe?.mission?.content2 ?? ''}</p>
          </div>
        ...
        >
          ...
          <h2 className="font-heading text-3xl font-bold text-text mb-2">{safe?.impact?.title ?? ''}</h2>
          <p className="text-gold font-heading text-lg mb-4">{safe?.impact?.subtitle ?? ''}</p>
          <p className="text-text-light leading-relaxed mb-6">{safe?.impact?.content ?? ''}</p>
          <div className="bg-white rounded-xl p-6 border-l-4 border-gold">
            <p className="text-text leading-relaxed italic">{safe?.impact?.callToWitness ?? ''}</p>
          </div>
        </motion.section>
        ...
        >
          ...
          <h2 className="font-heading text-3xl font-bold text-text mb-2">{safe?.businessModel?.title ?? ''}</h2>
          <p className="text-gold font-heading text-lg mb-2">{safe?.businessModel?.subtitle ?? ''}</p>
          <p className="text-text-light text-lg mb-8 italic">{safe?.businessModel?.philosophy ?? ''}</p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="bg-white rounded-xl shadow-md p-6 hover:shadow-lg transition-shadow">
              <Globe className="w-6 h-6 text-primary mb-3" />
              <h3 className="font-heading text-xl font-bold text-text mb-2">{safe?.businessModel?.freeAccess?.title ?? ''}</h3>
              <p className="text-text-light text-sm leading-relaxed">{safe?.businessModel?.freeAccess?.description ?? ''}</p>
            </div>
            <div className="bg-white rounded-xl shadow-md p-6 hover:shadow-lg transition-shadow">
              <Crown className="w-6 h-6 text-gold mb-3" />
              <h3 className="font-heading text-xl font-bold text-text mb-2">{safe?.businessModel?.premiumAccess?.title ?? ''}</h3>
              <p className="text-text-light text-sm leading-relaxed">{safe?.businessModel?.premiumAccess?.description ?? ''}</p>
            </div>
          </div>
          ...
          <div className="bg-primary-dark text-white rounded-2xl p-8 flex flex-col md:flex-row items-center gap-8">
            <div className="text-center">
              ...
              <p className="text-white/60 text-sm mt-1">of net profits</p>
            </div>
            <p className="text-white/80 leading-relaxed flex-1">
              {safe?.businessModel?.socialCommitment ?? ''}
            </p>
          </div>
        </motion.section>
