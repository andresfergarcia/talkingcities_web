...
export default function Header({ locale }: HeaderProps) {
  ...
  const renderLogo = () => {
    if (logoError) {
      return (
        <>
          <Headphones className={`w-7 h-7 transition-colors ${isScrolled ? "text-primary" : "text-white"} group-hover:text-gold`} />
          <span className={`font-heading text-xl sm:text-2xl font-bold transition-colors ${isScrolled ? "text-primary-dark" : "text-white"}`}>
            Talking Cities
          </span>
        </>
      );
    ...
    return (
      <div className="relative h-12 w-[180px] sm:h-14 sm:w-[210px]">
        {/* Color logo - visible when scrolled (white bg) */}
        <Image
          ...
          priority
        />
        {/* White logo - visible when not scrolled (transparent/dark bg) */}
        <Image
      ...
      <div className="max-w-content mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          <nav className="hidden md:flex items-center gap-1">
            ...
            <LanguagePicker currentLocale={locale} isScrolled={isScrolled} />
          ...
          >
            <nav className="px-4 py-4 space-y-1">
              ...
              <Link
                href="/tours"
                onClick={() => setIsMobileOpen(false)}
                className="block text-center mt-3 px-5 py-3 bg-accent hover:bg-accent-light text-white rounded-lg font-semibold transition-all"
              >
                {t("buyTour")}
              </Link>
            </nav>
